...
function createCursePages() {
	if (wikiDataLoaded) {
		const pageData = CURSES.map((_curse, curseID) => {
			return { name: wikiPageNames.curses[curseID], content: createCursePageContent(curseID) };
		});
		// Create item pages
...
function createAuroraPages() {
	if (wikiDataLoaded) {
		const pageData = AURORAS.map((_aurora, auroraID) => {
			return { name: wikiPageNames.auroras[auroraID], content: createAuroraPageContent(auroraID) };
		});
		// Create item pages
		bulkCreatePages(pageData, true);
	} else {
...
function createAncientMagickPages() {
	if (wikiDataLoaded) {
		const pageData = ANCIENT.map((_spell, spellID) => {
			return { name: wikiPageNames.ancient[spellID], content: createAncientMagickPageContent(spellID) };
		});
		// Create item pages
		bulkCreatePages(pageData, true);
	} else {
...
function createAltMagicPages() {
	if (wikiDataLoaded) {
		const pageData = ALTMAGIC.map((_spell, spellID) => {
			return { name: wikiPageNames.altmagic[spellID], content: createAltMagicPageContent(spellID) };
		});
		// Create item pages
		bulkCreatePages(pageData, true);
	} else {
...
function createPetPages() {
	if (wikiDataLoaded) {
		const pageData = PETS.map((_pet, petID) => {
			return { name: wikiPageNames.pets[petID], content: createPetPageContent(petID) };
		});
		// Create item pages
...
function updateVersion() {
	oldVersionReview.updatePages.push({ name: oldVersionReview.pages[oldVersionReview.currentPage].title, content: oldVersionReview.currentPageContent.replace(OLDVERSIONREGEX, VERSIONTEMPLATE) });
	proceedToNextPage();
}
